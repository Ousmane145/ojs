...
class SantaneAnalysisPlugin extends GenericPlugin {
    ...
    private function analyzeAbstract($abstract) {
        ...
        // Count sentences
        $sentenceCount = preg_match_all('/[^\s](\.|\!|\?)(?=\s|$)/', $abstract, $matches);
        
        // Check for keywords (simulated)
        $keywords = ['method', 'result', 'conclusion', 'study', 'research', 'data', 'analysis'];
        $foundKeywords = [];
        ...
        return [
            ...
            'recommendations' => $this->generateRecommendations($wordCount, $sentenceCount, count($foundKeywords), $clarityScore)
        ];
    }
    ...
    private function generateRecommendations($wordCount, $sentenceCount, $keywordCount, $clarityScore) {
        ...
        if ($wordCount < 150) {
            $recommendations[] = "Consider expanding your abstract. The ideal length is typically 150-250 words.";
        } elseif ($wordCount > 300) {
            $recommendations[] = "Your abstract may be too long. Consider condensing it to 150-250 words.";
        }
        
        if ($sentenceCount < 3) {
            $recommendations[] = "Your abstract may be too brief. Consider adding more detail about your methods and findings.";
        }
        
        if ($keywordCount < 3) {
            $recommendations[] = "Consider including more keywords relevant to your research area to improve discoverability.";
        }
        
        if ($clarityScore < 70) {
            $recommendations[] = "The clarity of your abstract could be improved. Consider simplifying complex sentences.";
        }
